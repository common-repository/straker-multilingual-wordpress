...
function qtrans_modifyCategoryForm($term) {
	qtrans_modifyTermForm('name', __('Category Name', 'straker'), $term);
	qtrans_modifyTermForm('tag-name', __('Category Name', 'straker'), $term);
}

function qtrans_modifyTagForm($term) {
	qtrans_modifyTermForm('name', __('Tag Name', 'straker'), $term);
	qtrans_modifyTermForm('tag-name', __('Tag Name', 'straker'), $term);
}

function qtrans_modifyLinkCategoryForm($term) {
	return qtrans_modifyTermForm('name', __('Category Name', 'straker'), $term);
}
...
/*	if($GLOBALS['wp_version'] != QT_SUPPORTED_WP_VERSION) {
		if(!(isset($_REQUEST['strakerincompatiblemessage'])&&$_REQUEST['strakerincompatiblemessage']=="shown")) {
			echo '<p class="updated" id="qtrans_imsg">'.__('The straker Editor has disabled itself because it hasn\'t been tested with your Wordpress version yet. This is done to prevent Wordpress from malfunctioning. You can reenable it by <a href="javascript:qtrans_editorInit();" title="Activate straker" id="qtrans_imsg_link">clicking here</a> (may cause <b>data loss</b>! Use at own risk!). To remove this message permanently, please update straker to the <a href="http://www.strakertranslations.com">corresponding version</a>.', 'straker').'</p>';
		}
		$init_editor = false;
	}*/
	// save callback hook
	...
	if(!user_can_richedit()) {
		//echo '<p class="updated">'.__('The straker Editor could not be loaded because WYSIWYG/TinyMCE is not activated in your profile.').'</p>';
		return $old_content;
	}
	...
	$content = apply_filters('straker_toolbar', $content);
	...
	$content_append = apply_filters('straker_modify_editor_js', $content_append);
	$content_append .="// ]]>\n</script>\n";
...
function qtrans_createTitlebarButton($parent, $language, $target, $id) {
	global $q_config;
	$html = "
		jQuery('#".$parent." .handlediv').after('<div class=\"straker_lang_div\" id=\"".$id."\"><img alt=\"".$language."\" title=\"".$q_config['language_name'][$language]."\" src=\"".WP_CONTENT_URL.'/'.$q_config['flag_location'].$q_config['flag'][$language]."\" /></div>');
...
function qtrans_insertTitleInput($language){
	...
	$html ="
		...
		var l = document.createTextNode('".__("Title", 'straker')." (".$q_config['language_name'][$language].")');
