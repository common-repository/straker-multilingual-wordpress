...
function qtrans_init() {
	...
	if(isset($_POST['straker_reset']) && isset($_POST['straker_reset2']) && defined('WP_ADMIN') && current_user_can('manage_options')) {
		// reset all settings
		delete_option('straker_language_names');
		delete_option('straker_enabled_languages');
		delete_option('straker_default_language');
		delete_option('straker_flag_location');
		delete_option('straker_flags');
		delete_option('straker_locales');
		delete_option('straker_na_messages');
		delete_option('straker_date_formats');
		delete_option('straker_time_formats');
		delete_option('straker_use_strftime');
		delete_option('straker_ignore_file_types');
		delete_option('straker_url_mode');
		delete_option('straker_detect_browser_language');
		delete_option('straker_hide_untranslated');
		delete_option('straker_auto_update_mo');
		delete_option('straker_next_update_mo');
		delete_option('straker_hide_default_language');
		if(isset($_POST['straker_reset3'])) {
			delete_option('straker_term_name');
		}
	}
	qtrans_loadConfig();
	...
	// set test cookie
	setcookie('qtrans_cookie_test', 'straker Cookie Test', 0, $q_config['url_info']['home'], $q_config['url_info']['host']);
	...
	if(strpos($q_config['url_info']['url'],'wp-login.php')!==false && defined('AUTH_KEY') && isset($wp_default_secret_key) && $wp_default_secret_key != AUTH_KEY) {
		global $error;
		$error = __('Your $wp_default_secret_key is mismatchting with your AUTH_KEY. This might cause you not to be able to login anymore.','straker');
	}
	*/
	...
	load_plugin_textdomain('straker', false, dirname(plugin_basename( __FILE__ )).'/lang');
...
function qtrans_loadConfig() {
	...
	// Load everything
	$language_names = get_option('straker_language_names');
	$enabled_languages = get_option('straker_enabled_languages');
	$default_language = get_option('straker_default_language');
	$flag_location = get_option('straker_flag_location');
	$flags = get_option('straker_flags');
	$locales = get_option('straker_locales');
	$na_messages = get_option('straker_na_messages');
	$date_formats = get_option('straker_date_formats');
	$time_formats = get_option('straker_time_formats');
	$use_strftime = get_option('straker_use_strftime');
	$ignore_file_types = get_option('straker_ignore_file_types');
	$url_mode = get_option('straker_url_mode');
	$detect_browser_language = get_option('straker_detect_browser_language');
	$hide_untranslated = get_option('straker_hide_untranslated');
	$auto_update_mo = get_option('straker_auto_update_mo');
	$term_name = get_option('straker_term_name');
	$hide_default_language = get_option('straker_hide_default_language');
...
function qtrans_saveConfig() {
	...
	// save everything
	update_option('straker_language_names', $q_config['language_name']);
	update_option('straker_enabled_languages', $q_config['enabled_languages']);
	update_option('straker_default_language', $q_config['default_language']);
	update_option('straker_flag_location', $q_config['flag_location']);
	update_option('straker_flags', $q_config['flag']);
	update_option('straker_locales', $q_config['locale']);
	update_option('straker_na_messages', $q_config['not_available']);
	update_option('straker_date_formats', $q_config['date_format']);
	update_option('straker_time_formats', $q_config['time_format']);
	update_option('straker_ignore_file_types', $q_config['ignore_file_types']);
	update_option('straker_url_mode', $q_config['url_mode']);
	update_option('straker_term_name', $q_config['term_name']);
	update_option('straker_use_strftime', $q_config['use_strftime']);
	if($q_config['detect_browser_language'])
		update_option('straker_detect_browser_language', '1');
	else
		update_option('straker_detect_browser_language', '0');
	if($q_config['hide_untranslated'])
		update_option('straker_hide_untranslated', '1');
	else
		update_option('straker_hide_untranslated', '0');
	if($q_config['auto_update_mo'])
		update_option('straker_auto_update_mo', '1');
	else
		update_option('straker_auto_update_mo', '0');
	if($q_config['hide_default_language'])
		update_option('straker_hide_default_language', '1');
	else
		update_option('straker_hide_default_language', '0');
		
	do_action('straker_saveConfig');
}
...
function qtrans_updateGettextDatabases($force = false, $only_for_language = '') {
	...
	$next_update = get_option('straker_next_update_mo');
	if(time() < $next_update && !$force) return true;
	update_option('straker_next_update_mo', time() + 7*24*60*60);
