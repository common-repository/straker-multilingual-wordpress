...
function qtrans_header(){
	...
	$css .="</style>\n";
	echo apply_filters('straker_header_css',$css);
...
function qtrans_adminHeader() {
	...
	echo "#straker_debug { width:100%; height:200px }";
	echo "#postexcerpt textarea { height:4em; margin:0; width:98% }";
	echo ".straker_lang_div { float:right; height:12px; width:18px; padding:6px 5px 8px 5px; cursor:pointer }";
	echo ".straker_lang_div.active { background: #DFDFDF; border-left:1px solid #D0D0D0; border-right: 1px solid #F7F7F7; padding:6px 4px 8px 4px }";
	do_action('straker_css');
	echo "</style>\n";
...
function qtrans_links($links, $file){ // copied from Sociable Plugin
	...
	static $this_plugin;
	if (!$this_plugin) $this_plugin = plugin_basename(dirname(__FILE__).'/straker.php');
	
	if ($file == $this_plugin){
		$settings_link = '<a href="options-general.php?page=straker">' . __('Settings', 'straker') . '</a>';
...
function qtrans_languageColumnHeader($columns){
	...
	$new_columns['language'] = __('Languages', 'straker');
...
function qtrans_languageColumn($column) {
	...
	if ($column == 'language') {
		...
		echo apply_filters('straker_available_languages_names',$available_languages_names);
		do_action('straker_languageColumn', $available_languages, $missing_languages);
	}
	return $column;
}
